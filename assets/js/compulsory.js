...
function confirmModal(message="Are you sure?") {
    ...
    <div data-label="confirmModalForm" class="flex space-x-1 my-2">
        <button id="confirmModalButtonOk" class="bg-green-600 text-white text-sm font-semibold px-1 py-0.5 rounded-sm outline-0 focus:ring-2 focus:ring-green-200">Ok</button>
        <button id="confirmModalButtonCancel" class="bg-red-600 text-white text-sm font-semibold px-1 py-0.5 rounded-sm outline-0 focus:ring-2 focus:ring-red-200">Cancel</button>
    </div>
</div>`;
if (!anyModalOpened) {
...
function alertModal(message="Alert", type="info") {
    ...
    <h3 class="text-sm font-semibold text-black">${message}</h3>
    <div data-label="confirmModalForm" class="flex space-x-1 my-2">
        <button id="confirmModalButtonOk" class="bg-${buttonColor}-600 text-white text-sm font-semibold px-1 py-0.5 outline-none rounded-sm outline-offset-0 outline-0 focus:ring-2 focus:ring-${buttonColor}-200">Ok</button>
    </div>
</div>`;
    if (!anyModalOpened) {
        anyModalOpened = true;
...
function promptModal(prompt="Enter value", defaultValue="", type=null, inputType="text", required=true) {
    ...
    <div data-label="promptForm" class="flex space-x-1 my-2">
        <button id="promptButtonOk" type="submit" class="bg-green-600 text-white text-sm font-semibold px-1 py-0.5 outline-none rounded-sm outline-0 focus:ring-2 focus:ring-green-200">Ok</button>
        <button id="promptButtonCancel" type="button" class="bg-sky-600 text-white text-sm font-semibold px-1 py-0.5 outline-none rounded-sm outline-0 focus:ring-2 focus:ring-sky-200">Cancel</button>
    </div>
</form>`;
    document.body.appendChild(element);
    if (type == "rename") {
